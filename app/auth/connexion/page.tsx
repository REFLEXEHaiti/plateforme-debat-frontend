...
export default function PageConnexion() {
  ...
  const soumettre = async (e: React.FormEvent) => {
    ...
    setErreur('');
    try { await seConnecter(email, motDePasse); }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #071220 0%, #0D1B2A 30%, #142236 60%, #0A1525 100%)', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 'clamp(40px,6vw,80px) clamp(16px,4vw,32px) 0', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 16, color: 'white' }}>
            <RubinMark size={28}/>
            <span style={{ fontFamily: 'Georgia,serif', fontSize: 24 }}>Débat sur <strong>Haïti</strong></span>
          </div>
        ...
        <form onSubmit={soumettre} style={{ width: '100%', maxWidth: 560, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <button type="submit" disabled={chargement} style={{ width: '100%', padding: '18px', background: chargement ? 'rgba(220,85,20,0.6)' : 'linear-gradient(135deg, #E8590C, #C0440A)', color: 'white', border: 'none', borderRadius: 100, fontFamily: "'Helvetica Neue',Arial,sans-serif", fontWeight: 700, fontSize: 18, cursor: chargement ? 'not-allowed' : 'pointer', boxShadow: '0 8px 32px rgba(220,85,20,0.45)', marginTop: 4 }}>
          ...
          <div style={{ textAlign: 'center', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, padding: '14px' }}>
            <span style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 15, color: 'rgba(255,255,255,0.6)' }}>Pas encore de compte ? </span>
            <Link href="/auth/inscription" style={{ color: '#FFB347', fontWeight: 800, textDecoration: 'none', fontSize: 15, fontFamily: "'Helvetica Neue',Arial,sans-serif", borderBottom: '2px solid rgba(255,179,71,0.4)', paddingBottom: 1 }}>
              S'inscrire gratuitement →
            </Link>
          </div>

          <div style={{ textAlign: 'center' }}>
            <a href="/" style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 13, color: 'rgba(255,255,255,0.4)', textDecoration: 'none' }}>
              ← Retour à l'accueil
            </a>
          </div>
        </form>
      ...
      <div style={{ position: 'relative', zIndex: 1, marginTop: 40, paddingBottom: 28 }}>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap', padding: '0 24px' }}>
          {['AyiboPost','YWCA Haïti','DECATHLON','eiffortic Haïti','Sun Auto'].map(p => (
            <div key={p} style={{ background: 'white', color: '#0D1B2A', padding: '8px 16px', borderRadius: 7, fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, fontWeight: 700, boxShadow: '0 2px 10px rgba(0,0,0,0.25)' }}>{p}</div>
          ))}
        </div>
      </div>
    </div>
